...
async def get_cash_fow_statement_parallel(
    tickers: List[str], 
    ...
    if os.path.exists("data/cash_fow_statements.json"):
        with open("data/cash_fow_statements.json", "r") as f:
            results = json.load(f)
    else:
        results = {}
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for i in range(0, len(new_tickers), batch_size):
            ...
            with open("cash_fow_statements.json", "w") as f:
...
async def main():
    ...
    # Process all tickers
    await get_cash_fow_statement_parallel(tickers)
