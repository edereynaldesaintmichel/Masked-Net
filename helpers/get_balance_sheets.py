...
async def get_balance_sheet_parallel(
    tickers: List[str], 
    ...
    if os.path.exists("balance_sheets.json"):
        with open("balance_sheets.json", "r") as f:
            results = json.load(f)
    else:
        results = {}
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for i in range(0, len(new_tickers), batch_size):
            ...
            with open("balance_sheets.json", "w") as f:
...
async def main():
    ...
    # Process all tickers
    await get_balance_sheet_parallel(tickers)
